...
RAD_paths = {'F:/MSc content/work/for Hussin/TT16_acrossDays/downsampled/TT16_Cont_10-31_downsampled.mat';
    ...
    'F:/MSc content/work/for Hussin/TT9_acrossDays/downsampled/TT9_Cont_11-06_downsampled.mat';
    'F:/MSc content/work/for Hussin/TT4_acrossDays/downsampled/TT4_Cont_10-27_downsampled.mat';
    'F:/MSc content/work/Dor/TT1/TT1_Cont_04-05_downsampled.mat';
    'F:/MSc content/work/Dor/TT11/TT11_Cont_04-09_downsampled.mat'; 
    'F:/MSc content/work/Dor/TT7/TT7_Cont_03-27_downsampled.mat';
    'F:/MSc content/work/Dor/TT12/TT12_Cont_04-05_downsampled.mat';
    'F:/MSc content/work/Dor/TT16/TT16_Cont_03-29_downsampled.mat';
};
...
filename = 'CasperandDor_RAD_ap.mat'
save(filename , 'RAD_ap')

data1 = load('CasperandDor_CA1_ap.mat'); % Replace 'file1.mat' with the actual file name
data2 = load('CasperandDor_RAD_ap.mat'); % Replace 'file2.mat' with the actual file name
...
% Group labels
group_labels = {'CA1', 'RAD'};

% Combine data for boxplot and scatter plot
combined_data = [CA1_numeric; RAD_numeric];
group = [ones(size(CA1_numeric, 1), 1); 2 * ones(size(RAD_numeric, 1), 1)];
...
for col = 1:2
    ...
    scatter(repmat(2, size(RAD_numeric, 1), 1), RAD_numeric(:, col), 'b', 'filled');
...
scatter(repmat(2, size(RAD_numeric, 1), 1), RAD_numeric(:, 3), 'b', 'o', 'filled');
...
scatter(repmat(4, size(RAD_numeric, 1), 1), RAD_numeric(:, 4), 'b', '^', 'filled');

title(['Exponents 1 & 2 (p1 = ', num2str(p_values(3)), ', p2 = ', num2str(p_values(4)), ')']);
xlabel('Group');
ylabel('Exponent Value');
xticks([1 2 3 4]);
xticklabels({'CA1 (Exp 1)', 'RAD (Exp 1)', 'CA1 (Exp 2)', 'RAD (Exp 2)'});
hold off;
...
p_values = zeros(1, num_columns);
for col = 1:num_columns
    [~, p_values(col)] = ttest(CA1_numeric(:, col), RAD_numeric(:, col));
end
