...
class FOOOF():
    ...
    def add_data(self, freqs, power_spectrum, freq_range=None, clear_results=True, z1_fix=None, d1_fix=None, d2_fix=None, off_fix=None, off_pr=0.0001, z1_pr=0.0001, d1_pr=0.0001, d2_pr=0.0001):
    ...
    def fit(self, freqs=None, power_spectrum=None, freq_range=None):
        ...
        try:
            ...
            #    print('ap_params_3exp: ', self.aperiodic_params_)
...
#################### new ap function with 3 options ###############################
    ...
    def _simple_ap_fit(self, freqs, power_spectrum):
        ...
        if self.aperiodic_mode == 'knee':
            ...
            if self.z1_fix is not None:
                guess[1] = self.z1_fix
                lower_bounds_l = list(ap_bounds[0])
                upper_bounds_l = list(ap_bounds[1])
                lower_bounds_l[1] = self.z1_fix - self.z1_pr
                if lower_bounds_l[1] < self.freq_range[0]:
                    lower_bounds_l[1] = self.freq_range[0]
                upper_bounds_l[1] = self.z1_fix + self.z1_pr
                upper_bounds_l[1] = self.z1_fix + self.z1_pr
                ap_bounds = (tuple(lower_bounds_l), tuple(upper_bounds_l))
            if self.d1_fix is not None:
                guess[2] = self.d1_fix
                lower_bounds_l = list(ap_bounds[0])
                upper_bounds_l = list(ap_bounds[1])
                lower_bounds_l[2] = self.d1_fix - self.d1_pr
                upper_bounds_l[2] = self.d1_fix + self.d1_pr
                ap_bounds = (tuple(lower_bounds_l), tuple(upper_bounds_l))
            if self.d2_fix is not None:
                guess[3] = self.d2_fix
                lower_bounds_l = list(ap_bounds[0])
                upper_bounds_l = list(ap_bounds[1])
                lower_bounds_l[3] = self.d2_fix - self.d2_pr
                upper_bounds_l[3] = self.d2_fix + self.d2_pr
                ap_bounds = (tuple(lower_bounds_l), tuple(upper_bounds_l))
            ...
            if aperiodic_params[2] > aperiodic_params[3]:
                d1_copy = aperiodic_params[2].copy()
                d2_copy = aperiodic_params[3].copy()
                aperiodic_params[2] = d2_copy
                aperiodic_params[3] = d1_copy
        ...
        if self.aperiodic_mode == "knee_flat":
            ...
            guess_d3 = [0]
            z2_guess = [(flat_off_guess[0] - off2)/(-1*new_guess_d2[0])]
            knee_off_guess = [np.log10(2) + (off1 - (guess_d1[0] * z1_guess[0]))]
            
            guess = np.array(knee_off_guess + z1_guess + z2_guess + guess_d1 + new_guess_d2) 
            ...
            if self.d1_fix is not None:
                ...
                lower_bounds_l = list(ap_bounds[0])
                upper_bounds_l = list(ap_bounds[1])
                lower_bounds_l[3] = self.d1_fix - self.d1_pr
                upper_bounds_l[3] = self.d1_fix + self.d1_pr
                ap_bounds = (tuple(lower_bounds_l), tuple(upper_bounds_l))
            if self.d2_fix is not None:
                guess[4] = self.d2_fix 
                lower_bounds_l = list(ap_bounds[0])
                upper_bounds_l = list(ap_bounds[1])
                lower_bounds_l[4] = self.d2_fix - self.d2_pr
                upper_bounds_l[4] = self.d2_fix + self.d2_pr
                ap_bounds = (tuple(lower_bounds_l), tuple(upper_bounds_l))
            if self.off_fix is not None:
                guess[0] = self.off_fix 
                lower_bounds_l = list(ap_bounds[0])
            ...
            if aperiodic_params[1] > aperiodic_params[2]:
                z1_copy = aperiodic_params[1].copy()
                z2_copy = aperiodic_params[2].copy()
                aperiodic_params[1] = z2_copy
                aperiodic_params[2] = z1_copy
        ...
        if self.aperiodic_mode == "knee_3exp":
            ...
            '''
            print('o: ', flat_off_guess)
            print('z1: ', z1_guess)
            print('z2: ', z2_guess)
            print('d1: ', guess_d1)
            print('d2: ', new_guess_d2)
            print('d3: ', d3_guess)
            '''
            guess = np.array(knee_off_guess + z1_guess + z2_guess + guess_d1 + new_guess_d2 + d3_guess) 
    ...
    def _robust_ap_fit(self, freqs, power_spectrum):
        ...
        #print('guess: ', popt)
        #print('bounds: ', ap_bounds)
        aperiodic_params, _ = curve_fit(get_ap_func(self.aperiodic_mode),
                                        freqs_ignore, spectrum_ignore, p0=popt,
                                        maxfev=self._maxfev, bounds=ap_bounds,
                                        ftol=self._tol, xtol=self._tol, gtol=self._tol,
                                        check_finite=False)
        if self.aperiodic_mode == 'knee_flat' or self.aperiodic_mode == 'knee_3exp':
            if aperiodic_params[1] > aperiodic_params[2]:
                    z1_copy = aperiodic_params[1].copy()
                    z2_copy = aperiodic_params[2].copy()
                    aperiodic_params[1] = z2_copy
...
##################################################inserted#########################################################################################
    ...
    def rem_electric_noise(self, power_spectrum, freqs): # input: logged psd and freqs 
        knee = True
        ...
        if knee:
            ap_bounds = ((-np.inf, -np.inf, -np.inf, -np.inf), (np.inf, np.inf, np.inf, np.inf)) 
        else: 
            ap_bounds = ((-np.inf, -np.inf), (np.inf, np.inf))
        

        psd_copy = power_spectrum.copy() # copy for use
        #psd_original = power_spectrum.copy() # copy for vis
        ...
        """
        noise_bands = [[43, 57],  [12.5, 22]]
        correnction_bands = [[48.5, 51.5],  [13.5, 21.5]]
        """
        ...
        for noise_band, correction_band in zip(noise_bands, correnction_bands):
            ...
            n_off_guess = [off1]
            n_exp_guess = guess_d1
        #  print(freqs_n)
        #  print('psd:')
        #  print(psd_n)
            noise_guess = np.array(n_off_guess + n_exp_guess) 
            noise_ap_params0, _ = curve_fit(get_ap_func(knee),
            ...
            noise_ap_params1, _ = curve_fit(get_ap_func(knee),
            ...
            if guess_height > 0.01: # min_peak_height arbitrarily chosen to be 0.2
                ...
                #guess_n_peak = _drop_peak_cf(guess_n_peak, noise_band) # maybe should be removed
        
                if len(guess_n_peak) > 0:
                    #gaussian_params = self._fit_peak_guess(guess_n_peak)
                    ...
                    noise_ap_params2, _ = curve_fit(get_ap_func(knee),
                    ...
                    power_spectrum[f_mask_n] = power_spectrum[f_mask_n] - noise_peak #--> not wrong, but not always 100% effective
                    #plt.figure()
                    #plt.plot(freqs_n, noise_peak)
                    #plt.show()
                    #psd_copy[f_mask_n] = iters_noise_fit
                    #power_spectrum[f_mask_correction] = psd_copy[f_mask_correction]
...
#########################new fit_peaks function####################
    ...
    def _fit_peaks(self, flat_iter):
        ...
        ''' 
        # includes noise peaks
        self.std_limits = np.empty([0 , 2])
        self.cf_bounds = np.empty([0 , 2])
        theta_range = [self.freq_range[0], 19]
        theta_harmonic_range = [11, 20]
        sgamma_range = [19, 48]
        fgamma_range = [51, 90]
        ripple_range = [100, 200]
        electric_noise_range = [43, 57]
        electric_harmonic_range1 = [145, 155]
        list_of_ranges = [electric_harmonic_range1, electric_noise_range, ripple_range, fgamma_range, theta_range, theta_harmonic_range, sgamma_range]  
        list_of_thresholds = [ 1.5, 0.5, 1, 1, 1, 0.3, 1]
        abs_thresholds = [ 0.1, 0.08, 0.05, 0.08, 0.1, 0.05, 0.05]
        list_of_peak_widths = [[0.2, 3], [0.2, 3], [10, 30], [3, 35], [2, 5], [0.25, 4], [5, 20]]
        allowed_ranges = [[149, 151], [49, 51], [125, 160], [52, 80], [5, 9.5], [14, 19] , [25, 47]]
        # Initialize matrix of guess parameters for gaussian fitting
        guess = np.empty([0, 3])
        '''
        ...
        list_of_thresholds = [1, 1, 1, 1]
        abs_thresholds = [ 0.05, 0.08, 0.1, 0.05]
        list_of_peak_widths = [ [10, 30], [3, 35], [2, 5], [5, 20]]
        allowed_ranges = [[125, 160], [52, 80], [5, 9.5], [25, 40]]
        # Initialize matrix of guess parameters for gaussian fitting
        guess = np.empty([0, 3])


        #added for high freq noise peak in Einstein CA1 tetrodes
        if self.freq_range[1] >= 400:
            mysterious_peak_range = [205, 495]
            list_of_ranges = [ripple_range, fgamma_range, theta_range, sgamma_range, mysterious_peak_range]
        ...
        '''
        # for 4-500Hz # dont use this. let's keep the ripple range the same over different fitting ranges 
        self.std_limits = np.empty([0 , 2])
        self.cf_bounds = np.empty([0 , 2])
        theta_range = [self.freq_range[0], 19]
        theta_harmonic_range = [11, 20]
        ...
        abs_thresholds = [ 0.05, 0.08, 0.1, 0.05]
        list_of_peak_widths = [ [10, 30], [3, 35], [2, 5], [5, 20]]
        allowed_ranges = [[125, 210], [52, 80], [5, 9.5], [25, 47]]
        # Initialize matrix of guess parameters for gaussian fitting
        guess = np.empty([0, 3])
        '''
        ...
        #######for 100 Hz ############ includes noise peaks
        """
        self.std_limits = np.empty([0 , 2])
        self.cf_bounds = np.empty([0 , 2])
        theta_range = [self.freq_range[0], 19]
        ...
        electric_noise_range = [43, 57]
        electric_harmonic_range1 = [145, 155]
        list_of_ranges = [electric_noise_range, fgamma_range, theta_range, theta_harmonic_range, sgamma_range]  
        list_of_thresholds = [  0.5, 1, 1, 0.5, 1]
        abs_thresholds = [  0.08, 0.05, 0.08, 0.1, 0.03, 0.05]
        list_of_peak_widths = [ [0.2, 1], [3, 35], [2, 5], [0.25, 4], [5, 20]]
        allowed_ranges = [[49, 51], [52, 80], [5, 9.5], [14, 19] , [25, 47]]
        # Initialize matrix of guess parameters for gaussian fitting
        guess = np.empty([0, 3])
        """
        # Find peak: Loop through, finding a candidate peak, and fitting with a guess gaussian
        ...
        for allowed_range, current_range, current_threshold, current_width, abs_threshold in zip(allowed_ranges, list_of_ranges, list_of_thresholds, list_of_peak_widths, abs_thresholds):
            ...
            #########newly inserted start###################### # for early exclusion of not expected peaks and more appropriate functioning of thresholding
            prev_guess_freq = guess_freq.copy()
            if guess_freq < allowed_range[0]:
                guess_freq = np.mean(allowed_range) #allowed_range[0]
                print('guess_cf corrected from ', prev_guess_freq, ' to ', guess_freq)
                max_ind = int(np.round((guess_freq - current_range[0])/self.freq_res))
                max_height = current_band[max_ind]
            if guess_freq > allowed_range[1]:
                guess_freq = np.mean(allowed_range) #allowed_range[1]
                print('guess_cf corrected from ', prev_guess_freq, ' to ', guess_freq)
                max_ind = int(np.round((guess_freq - current_range[0])/self.freq_res))
                max_height = current_band[max_ind]
            #########newly inserted end######################
